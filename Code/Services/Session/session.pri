...
# 「创建工厂」只前向声明 Views 层类型、不 include 它的直接收益。
INCLUDEPATH += $$PWD

# 类型掩码复用 Services/Filter 的掩码语言（mask.h / FILT-001），因此要能看到它。
# SESS-006 的掩码清单项同样复用它的**解析器**（maskfilter.h），理由不变。
#
...
# Filter 的 .pri，否则会链接报「Mask::compile 未定义」。Tests/Session、
# Tests/SessionType 与 Tests/Settings 都照做了，并在各自的 .pro 里写了原因。
