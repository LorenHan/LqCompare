...
| `Session/` | `session.{h,cpp}` | 会话设置的抽象接口 `SessionSettings` 与一个内存实现 `MemorySessionSettings`（SESS-001 契约里的 `sessionSettings`）；作用域链与落盘留给 SESS-006 / SESS-007 | `session.pri` |
...
| 模块 | 承担的功能域 |
| --- | --- |
| `Session/` | 会话抽象基类（**已落地**，横跨 `Views/Session/` 与 `Services/Session/` 两段，见 §3.7）、类型注册表、设置作用域、会话文件读写 |
