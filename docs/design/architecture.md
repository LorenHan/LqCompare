...
| `Filter/` | `mask.{h,cpp}`、`maskfilter.{h,cpp}`、`attributefilter.{h,cpp}` | 掩码语言（`*` / `?` / `[...]` / `**` 的解析与匹配、转义、语法速查）与过滤声明（包含/排除的叠加、排除优先、大小写策略、预览计数）（FILT-001）；**属性条件**（大小范围 / 修改时间范围 / 属性位 / 所有者与组、条目元数据快照 `EntryMetadata`、条件表自检、`decideEntry()` 把名称侧与属性侧合成**与**）（FILT-003） | `filter.pri` |
...
前三段只依赖 QtCore（输入输出都是字符串，不碰文件系统、不碰图形、不碰平台 API），
第四段多依赖一个 `Services/Session` 的**存储抽象**（一个纯 QtCore 的接口）：
...
**四段刻意分开**，因为它们回答的是四个问题、失败也是四类：`mask.h` 回答「这一段
