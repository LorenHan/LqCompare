...
| `Text/` | 行对齐算法（**已落地**：Myers / Patience，见 §3.3）、忽略规则（**已落地**：大小写折叠与两级空白，见 §3.3）、相似行对齐与阈值（**已落地**：TXT-005，见 §3.3）、编码探测、字符级差异 |
| `Merge/` | 三方合并引擎与冲突判定 |
