...
| （夜间新增的其余服务模块） | `Text/`、`Folder/`、`Merge/`、`Special/`、`Table/`、`Sync/`、`Snapshot/`、`Version/`、`Media/`、`Registry/`、`FolderMerge/`、`Archive/`、`Cli/`、`Script/`、`Settings/` | 比对引擎与各专用类型后端、目录同步与快照、命令行与脚本、设置存储等。**逐条的公开接口与未覆盖范围见各自的 `docs/development/team-*.md`** | 各目录下的 `.pri` |
