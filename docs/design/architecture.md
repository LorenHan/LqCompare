...
其中四段只依赖 QtCore（输入输出都是字符串，不碰文件系统、不碰图形、不碰平台 API；
`namefilter` 另用 QtCore 的线程原语做超时保护），只有 `filterstack` 多依赖一个
`Services/Session` 的**存储抽象**（一个纯 QtCore 的接口）：
...
**五段刻意分开**，因为它们回答的是五个问题、失败也是五类：`mask.h` 回答「这一段
