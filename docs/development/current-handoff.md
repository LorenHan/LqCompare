...
| 测试（全量） | **3598 passed / 0 failed / 2 skipped，66 个套件**（2026-09-21 11:38 实测，**并行 4 时 1 分 21 秒**，EXIT=0）。并行化前后**逐项相同**——数字没变是预期的：本轮补的是运行器缺的两条完成标准与一处违反，没有增删用例。串行基线（同一台机器、同一轮代码）是 **3 分 53 秒**，所以并行的收益是 233s → 81s。2 条跳过分别来自 `PathName`（40/0/1）与 `Registry`（61/0/1），都是按平台条件跳过的用例 | `Code/Tests/run-tests.sh` |
...
> `a1cd3ee`、`786cf46`、`24225e8`、`525a872`、`0ba8476`、`6cd5831`、`6bd1385` 与 `810262f` 这二十二行由
> **单独的纯文档提交**补写提交号，原因见下一段——把一个提交的提交号写进它自己，
> 会因为 `--amend` 每次都改变提交号而永远对不上。
> 补写链**只到代码提交为止**：补写这些行的那个纯文档提交自身不再上表，否则每轮都会多出一行
> 永远指不到自己的记录（`bf05600` 就是这样一个未被记录的补写提交）。
> 本轮新增的 `5c16735` 走的是同一条路（提交号写在本节的最后一行，由紧随其后的纯文档提交定稿）。
...
> **最新一轮（2026-09-21 11:xx）闭环了 `ENG-003`（测试框架与测试运行器，issue #335），
> 见 §4.0.3 与 §1.24**——它补的是运行器缺的两条完成标准（并行、超时）与一处违反
> （测试往仓库根写截图），并给运行器加了 `--self-test`。
> 下面的 §4.0 / §4.0.1 / §4.0.2 是更早几轮的记录，**优先级表以 §4.0.3 的结尾为准**。
...
### 4.0.3 最新一轮（2026-09-21 11:xx）：ENG-003 测试框架与测试运行器（issue #335）

**先读 §1.24**，那里有完整的交付记录（五条完成标准逐条对着源码核对的结果、
三个新增能力的设计理由、14 处变异、以及第一次真漏检的成因）。这里只写
**下一轮该从哪儿开始**。
