...
| 测试（全量） | **3622 passed / 0 failed / 2 skipped，67 个套件**（2026-09-21 14:2x 实测，并行 4 时约 1 分 40 秒，EXIT=0）。3622 = 3615（TXT-003 那一轮，见 §1.26）+ 7（TXT-008 本轮新增：`Tests/Text` 6 个 + `Tests/TextView` 1 个，见 §1.27），**没有删改任何既有用例**。2 条跳过分别来自 `PathName`（40/0/1）与 `Registry`（61/0/1），都是按平台条件跳过的用例 | `Code/Tests/run-tests.sh` |
...
> `a1cd3ee`、`786cf46`、`24225e8`、`525a872`、`0ba8476`、`6cd5831`、`6bd1385`、`810262f`、`3b3a993` 与 `5af6b9e` 这二十四行由
> **单独的纯文档提交**补写提交号，原因见下一段——把一个提交的提交号写进它自己，
> 会因为 `--amend` 每次都改变提交号而永远对不上。
> 补写链**只到代码提交为止**：补写这些行的那个纯文档提交自身不再上表，否则每轮都会多出一行
> 永远指不到自己的记录（`bf05600` 与 TXT-003 那一轮的补写提交都是这样未被记录的）。
> 本轮 `5af6b9e` 走的是同一条路：代码提交里只写「提交号见 §3.1」，
> 由紧随其后的纯文档提交把这一行与 §1.27 的末句一起定稿。
