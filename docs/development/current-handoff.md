...
- **Git 差异集成（`Services/Vcs/`、`Views/Vcs/`）**：`vcsbackend.h` 是只读后端
  ...
  `vcsavailability.h` 回答另外两个问题——「这条命令现在能不能用」（命令可用性的
  **唯一定义与唯一翻译处**）与「同一个路径要不要再问一遍后端」（按规范化绝对路径分桶、
  只缓存定论、锁外探测 + 代次校验）；`vcsview.h` 有 HEAD / 索引 / 修订 / 日志四种模式；
  `blameview.h` 是逐行追溯视图
...
| `Services/Vcs/`（后端抽象层） | VCS-001 | **已完成**（五条标准全落，见 §1.42）：接口七件套 `detectRepo` / `status` / `log` / `diff` / `blame` / `revisionGraph` / `catFile` 都在抽象类上；`GitBackend` 调**用户安装的外部 git 进程**（独立参数 + `GIT_*` 环境清洗 + 超时与输出上限，**不引入任何仓库访问库**，因此也不产生新的许可结论）；**无障碍降级**由新模块 `vcsavailability.{h,cpp}` 承担——`probeAvailability()` 把后端的 `availability()` 折成「能不能用 + 为什么」，`MainWindow::updateCommandState()` 按 `isVcsActionId()` 把三条 VCS 命令**一起**置灰并给出同一句「未检测到 git…」；**按路径缓存**由 `RepositoryCache` 承担（规范化绝对路径分桶、只缓存定论、锁外探测 + 代次校验丢弃「换后端那一刻在途」的结果），视图跨模式切换保留、显式刷新才失效；**可替换后端**由 `VcsView::setBackend()` / `MainWindow::setVcsBackend()` 提供，三个套件都在没有 git 的假后端上跑 | `Tests/Vcs`（68 个用例，本轮 58 → 68）+ `Tests/VcsView`（17，16 → 17）+ `Tests/AppIntegration`（14，13 → 14） |
| `Services/Vcs/`（其余 VCS 条目的共同底座） | VCS-002 ~ VCS-019 | **部分完成**（HEAD / 索引 / 任意修订 / 冲突三阶段 / 日志 / 引用 / blame / 修订图数据均可用；无引用选择浏览器、merge-base、日志正则搜索、图形列） | 同上 |
...
| 测试（全量） | **3775 passed / 0 failed / 2 skipped，71 个套件**（2026-09-24 04:5x 实测，EXIT=0）。3775 = 3763（DIR-012 那一轮，见 §1.39）+ 12（VCS-001 本轮：`Tests/Vcs` 58 → 68、`Tests/VcsView` 16 → 17、`Tests/AppIntegration` 13 → 14，**没有新增套件**）；3763 = 3730（DIR-003 那一轮，见 §1.38）+ 33（DIR-012：**新套件** `Tests/StatusPalette` 29 个用例函数 + `Tests/Folder` 51 → 55）；3730 = 3724（DIR-011 那一轮，见 §1.37）+ 6（DIR-003：`Tests/Folder` 45 → 51，**没有新增套件**）；3724 = 3688（DIR-008 那一轮，见 §1.33）+ 36（DIR-011：**新套件** `Tests/EntryStatus` 27 + `Tests/Folder` 35 → 45）；此前的 3688 = 3681（TXT-012 那一轮，见 §1.32）+ 7（`Tests/Folder` 30 → 36、`Tests/Report` 36 → 37），**没有删改任何既有用例**。2 条跳过分别来自 `PathName`（40/0/1）与 `Registry`（61/0/1），都是按平台条件跳过的用例 | `Code/Tests/run-tests.sh` |
| 运行器自己也有红有绿（ENG-003 第 2、3 条，§1.34 从 24 涨到 30，**§1.35 起是 32，§1.40 起是 46、补上日志断言后是 48**） | `run-tests.sh --self-test` 在**临时目录**里现造**六个**探针套件（通过 / 失败 / 挂死 / 硬退出 / **有信号地崩掉** / 构建期失败）**外加一个假调试器脚本**，用同一个运行器跑两遍（并行 4 与串行 1），**48 条断言全绿**；并发度实测**并行峰值 2 / 串行峰值 1**。挂死探针**自己先写一份统计行再挂死**（否则「超时不计入合计」这条边界在 macOS 上根本不可观察，见 §1.35），两遍的 `合计：6 passed, 1 failed, 0 skipped` 逐字相同。**证据在每一遍结束时立刻按遍快照**（`evidence-jobs4.txt` / `evidence-jobs1.txt`）——两遍共用一个构建目录，不快照就只剩第二遍的。除这两遍外还有**两遍只跑崩溃探针**的调用（过滤器 + 独立构建根目录）：一遍验 lldb 的旗标形状、一遍验「本平台没有可用调试器」，这两支平时不会走到、写错了却会让整条诊断静默失效。探针与假调试器跑完即删、不进仓库（`LQCOMPARE_SELFTEST_KEEP=1` 可留现场）。自测同时是超时保护、崩溃诊断与「产物必须先删」三条规则的**唯一可重复证据**（它们只在失败路径上才看得出来） | `Code/Tests/run-tests.sh --self-test` |
| 崩溃栈捕获能反向验证（§1.40，接手后本机重跑见 §1.41） | **15 处变异、14 处检出、1 处按设计不漏检、0 处漏检**（另有 5 处除预期断言外还牵红了别的断言，逐条看过都是可解释的下游后果）。检出的是：删掉抓栈调用、把范围放宽成「所有失败的套件」（本该被「**只**对崩溃的套件抓栈」那条钉住）、gdb 旗标里去掉 `-ex bt`、lldb 旗标里去掉 `-o bt`、给程序加 `-o results.txt`（⇒ 程序自己的输出不再落进产物，这正是「诊断不改写证据」的可观测面）、帧正则改成永不匹配、删掉「跑完了但没拿到栈」那一支结论、不检查调试器是否可执行、没有调试器时不留产物、降级文案里去掉环境变量、降级不打日志、每轮开头不清上一轮的 `crash-trace.txt`、探测不查 `PATH`。**按设计不漏检的一处**：把产物从覆盖（`>`）改成追加（`>>`）——每轮开头已经 `rm -f` 过，两者在任何输入下都不可区分 | 变异测试（驱动 `/tmp/lqcompare-mutate-crashtrace.py`，**不进仓库**）；被测对象是 shell 脚本，所以**每处变异都要重跑一遍完整自测**（约 56 秒），不需要碰 `.o` 的 mtime |
...
### 1.40 崩溃套件的调用栈捕获：把 ubuntu 那条崩溃线从「哪一条用例」推进到「哪一行」（2026-09-24 01:2x）

**这一轮做的事与它为什么排在第一位**：§1.36 把 `Tests/Folder` 在 ubuntu 上的
`*** buffer overflow detected ***` 定位到了「崩在用例 `linksAreComparedWithoutFollowing()`、
且是在 `Folder::compare()` 这一次调用里」，然后就停住了——因为那串调用
（`enumerateDirectory` → 符号链接判定 → `linkTarget()` → `canonicalFilePath()`）
里既有本仓代码也有 Qt / glibc，**凭猜改一处的代价是「改错 + 一次 CI 往返」**。
§1.36 当时列了三个候选动作，本轮落地第 2 个：**要一份调用栈**。

**新增的能力**（全在 `Code/Tests/run-tests.sh`，没有一行生产代码改动）：
崩溃的套件（判据与 `-v2` 补跑**完全相同**：非超时 + 没产出 `Totals:` 行）
会被调试器再跑一次，产物是 `${build_dir}/crash-trace.txt`，里面依次是
「首轮退出码 / 用了哪个调试器 / 完整命令行 / 看门狗秒数 / 调试器与进程的原始输出 /
**我们自己写的结论行**」。产物随 CI 的测试日志一起上传（`.github/workflows/build.yml`
的 `_test-build/**/crash-trace.txt`），CI 在跑测试之前还会先打一行「崩溃栈捕获可用调试器：…」，
没有调试器时打 `::warning::`（**不让这条腿变红**：缺调试器是环境差异，不是被测对象失败）。

**三条纪律，每条都对应一个会真实发生的错法**：

1. **用同一个崩溃判据**。有 `Totals:` 行的失败，用例名已经在 `results.txt` 里；
   超时的套件上面已经单独点名，再挂一个调试器只会白等一个超时。
2. **不传 `-o`**。传了就是把首轮那份「崩之前跑过哪些用例」的记录改写掉——
   等于让诊断改写证据。副产品很有用：程序自己的输出（Qt Test 默认写 stdout）会落进
   `crash-trace.txt`，于是「调试器真的把二进制跑起来了」是**可断言的**，不靠信任。
3. **超时下限 60 秒**（`max(TIMEOUT, 60)`）。调试器要先把自己启动起来，这跟套件快不快
   无关；自测的超时窗口是 5 秒，不加下限的话「调试器还没启动完就被看门狗掐掉」会把
   「没抓到栈」伪装成「没有栈可抓」。**实测这条下限救的就是本机**（见下）。

**没有可用调试器时照样写下 `crash-trace.txt`**，里面明说抓不到栈、探了哪两个、可以用
`LQCOMPARE_TEST_DEBUGGER` 覆盖。这样「产物存在」始终等价于「这个套件崩过」
（与 `verbose.txt` 同一约定），而「里面没有栈」永远是一句**写出来的结论**。

**结论行由我们自己写**，不把调试器的原始输出一丢了之：lldb 在进程已被信号带走时只回
一句 `error: Command requires a process which is currently stopped`，gdb 在没有调试
信息时只有地址，**「调试器根本没停住」与「停住了但栈是空的」会长成同一个样子**。
所以三种结局分开写：拿到 N 帧 / 调试器跑完了但一帧都没有 / 看门狗终止。

**自测为什么用一个「假调试器」**（这条最容易被误读成偷懒，所以写清楚）：
本机的 lldb **调试不通 x86_64（Rosetta）进程**，两条实测——
进程被信号带走时它不会停（`-o bt` 因此拿不到栈），在 `abort` 上打断点也不命中；
而同一台机器上一个原生 arm64 的 abort 程序会让 lldb **直接卡在 `run` 上不动**
（debugserver 起来之后没有下文）。所以自测验的是**运行器这一侧**的契约：
给了调试器就必须按「要调用栈」的形状调它、必须把输出与结论写进产物、必须只对崩溃的
套件这么做、没有调试器时必须明说。**真调试器那一支只能在 CI 上跑，本机不当作已验证**
（见下面「还没做到的」）。

**测试与验证**：
- `run-tests.sh --self-test`：**32 → 46 条断言**，补上下面那条「日志帧清单」后是 **48 条**。
  探针从 5 个涨到 **6 个**（新增
  `ZZProbeAbort`：把 `SIGABRT` 的处置复位成默认再 `raise`，因此**每个平台**都走
  「崩溃」这条路；用 `raise` 而不是 `abort()` 是因为 `abort` 是 `noreturn`，`-O2`
  下可能被编译成尾调用、把「崩在哪个函数」那一帧抹掉）。
  除两遍（并行 4 / 串行 1）外，另加**两遍只跑崩溃探针**的过滤器调用，各配一个独立构建根目录
  （共用目录会把上两遍的产物覆盖掉）：一遍验 lldb 的旗标形状，一遍验「本平台没有调试器」。
- 新增的 16 条断言覆盖：产物留下 / 记下调试器 / **gdb 与 lldb 两种旗标形状** /
  程序自己的输出在产物里（= 没传 `-o`）/ 帧里出现崩溃点所在函数 / 结论说拿到了栈 /
  **拿到栈时日志里点出帧数并贴出帧**（见下面那条自查发现的缺口）/
  没停住时结论照实说没拿到 **/ **只有**崩溃的套件才抓栈（四个不崩的探针都没有产物）/
  降级三件（产物照旧留下、明说没拿到、说清怎么才能拿到、日志里也有一份）/
  以及「上一轮的 `crash-trace.txt` 必须在本轮开头被删掉」。
- **收尾自查发现一个真实缺口并当场补上**：按「每一处实现都要问『谁能把它打红』」这条
  纪律扫 `capture_crash_trace()` 的每个分支，发现**拿到调用栈时那份运行器日志**
  （`调用栈 N 帧` + `| ` 前缀贴出的几帧）**没有任何断言守着**——把整段删掉，原来的
  46 条断言全绿。而那段日志正是「不必下产物就能看到」这句承诺的全部兑现，删掉不会
  变红的分支就是没人知道的死代码（TXT-010 / DIR-003 的同一条判据）。补了 2 条断言
  （帧数 + 真贴出了带 `|` 前缀的帧，都走**专跑崩溃探针那一遍**的 `out-lldb.log`，
  避免与全量两遍的并行/计数断言抢同一个证据来源），并加了一处变异（M15）反向验证它。
- **本机端到端实测一次真 lldb**（`LQCOMPARE_TEST_DEBUGGER=/usr/bin/lldb`）：lldb 卡在
  `run` 上不返回，**被 60 秒下限的看门狗终止**，产物按设计写下
  「结论：调试器在 60s 内没有结束，已被看门狗终止——没有拿到调用栈」。
  这一条同时证明了「下限」与「结论照实写」两件事都是真的在起作用。
- 五道护栏全绿（`check_icons` 41 个图标 / `check_winapi` 351 个源文件 /
  `check_spec` 369 条、P0 59 条 / `check_shell` 16 个脚本 / 分层检查通过）。
- 全量与主程序构建的数据见 §1.41 的表格（那一轮把本节的工作验证完并提交）。

**还没做到的（下一轮别当成已完成）**：

- **`gdb` 那一支没有在本机实测过**。本机（macOS）没有 gdb，只有 lldb，而 lldb 在这台
  机器上不通。自测里 gdb 的形状是**假调试器**收下的（它把收到的 argv 原样记进产物，
  所以断言钉的是真实调用形状，不是运行器自己打印的一行），**真 gdb 只能等 ubuntu 腿
  第一次跑到崩溃时才知道**。所以这一轮**没有**动 `foldercompare.cpp`——
  §1.36 那句「在拿到结论之前不要改」依然有效。
- **macOS 上拿不到栈**：本机 lldb 的两种不通法都会走到「跑完了但没拿到调用栈」或
  「超时」这两支结论。这不是运行器的问题，但要知道 mac 腿上的 `crash-trace.txt`
  目前**只会是一句说明**。
- **帧名靠符号表，不靠调试信息**：测试工程是 `-O2` 且**没有 `-g`**（实测生成的
  `Makefile` 里 `CFLAGS/CXXFLAGS` 都没有 `-g`），所以帧名来自符号表——
  非静态函数有名字、`static` 匿名命名空间里的函数可能只有地址。真要更精确的帧，
  得给那一支另加 `-g` 重编，那是另一件事。
- **调用栈这一层能不能真的把 `Folder::compare()` 那一串指到某一处，要等下一次 CI**
  （这正是本轮唯一的目的）。拿到之后按 §1.36 的候选动作 1（二分夹具）或直接改代码走。

### 1.41 接手一个被中断的会话：把 §1.40 的调用栈捕获**验完并提交**（2026-09-24 02:5x）

**为什么会有这一节**：§1.40 那一轮跑到一半被强杀，产物全在工作树里、**一次都没提交、
也没做过一次完整验证**。本轮第 0 步判锁时发现 `run.lock` 的 `pid=43189` 已死
（`kill -0` 失败，且 `heartbeat` 停在 `started` 上、从头到尾没刷新过一次），
`pgrep` 里没有别的 `--session-id`，12 分钟窗口内无改动，`git log -1` 在两小时前 ⇒
判为「上一轮被中断」并接手（pid 写 `$PPID`；沙箱里 `ps` 被禁、`$$` 拿到的 pid 立刻变死，
会被下一轮误判成死锁）。**收工前已删锁、工作树干净、`HEAD` 与远端 `main` 一致。**

**接手前先逐文件判断「这是半成品还是错误状态」**（判据见 §4.1 与 §6）：工作树里是
4 个文件、551 行插入（`Code/Tests/run-tests.sh` +386、`.github/workflows/build.yml`、
`docs/design/architecture.md`、`docs/development/current-handoff.md`）。核对结论是
**可用的成品**：三个新函数（`select_crash_debugger` / `crash_trace_argv` /
`capture_crash_trace`）齐全且已接进 `run_suite()` 的崩溃分支，自测的 16 条新断言齐全，
文档连 §6 的坑表都写了。因此**不回滚**，直接在它之上做验证。

**接手后第一件事是证明「没有变异残留」，而且不能用 grep 特征字符串来证明**：
把 `Code/Tests/run-tests.sh` 与上一轮驱动在基线绿之后留下的
`/tmp/lq-run-tests-pristine.sh` 做 `diff` —— **逐字节相同**（sha256 都是 `c742e43…`），
说明那次 SIGKILL 之后源码已被还原干净。这一步必须这么做，因为
**「没有残留」不能靠枚举变异特征来证明**（§6 那条：上一轮就是只 grep 了 M02 / M06 的特征、
而真正留住的是 M01，于是检查本身给出了绿灯，白跑一轮自测才定位到）。

**本轮全部验证都是本机实跑，不是抄上一轮的结论**：

| 验证项 | 实测结果 |
| --- | --- |
| `run-tests.sh --self-test` | **48 条断言全绿**（静态清点：49 个 `st_*` 调用点减去 `st_contains()` 体内那次委托 = 48，与运行结果一致）；探针 **6 个**；两遍（并行 4 / 串行 1）合计逐字相同 `合计：6 passed, 1 failed, 0 skipped`，并发峰值实测 **2 / 1** |
| 全量测试 | **3763 passed / 0 failed / 2 skipped，71 个套件，EXIT=0**（`grep -c '^▶ '` 数出 71 个套件头，逐套件 `Totals:` 行相加也是 3763/0/2） |
| 五道护栏 | 全部 `exit=0`：`check_layering` 分层通过 / `check_winapi` 351 个源文件 / `check_spec` 369 条规格、P0 59 条 / `check_icons` 41 个图标 / `check_shell` 16 个脚本。**`exit=` 取自护栏程序本身**，不是管道末端 `tail` 的 |
| 主程序构建 | **全量重编 105 个翻译单元、0 条本仓 warning**，`make` 退出码 0，二进制重新链接（02:58） |
| 主程序启动 | 重签后 `QT_QPA_PLATFORM=offscreen … --new-instance` 三行日志齐全：单实例机制已由选项关闭 → Ribbon 构建完成：10 页 / 45 组 / 169 个按钮 → LqCompare 0.1.0 启动完成；跑完逐个 `kill` |
| 变异（反向验证） | **15 处变异、14 处检出、1 处按设计不漏检、0 处漏检**（本机重跑第一手；上一轮写成「14 处 13 处」，本轮按实跑结果改正，见下面「勘误」） |

**勘误一处**：`docs/design/architecture.md` 的 `Tests/` 行此前写「71 个套件 / 3762 条」，
而实测 passed 是 **3763**（DIR-012 那一轮留下的偏差），本轮把那个数字改正。
**这一处只改数字、不改形状**——`check_spec.py` 有一条保留写法把「全角括号 + 数字 + 条」
当成规格条目数，改形状会连带把它打红。

**本轮最值钱的一条（工具坑，会重复踩）**：**`make clean` 在全量测试之后会静默失败，
于是「全量重编」其实一次都没编**。沙箱的批量删除守卫**按轮累计**（阈值 50），
全量测试的产物清理早已把配额用掉，于是 `make clean` 的每一条 `rm -f` 都被拦
（`[safe-delete][SAFE_DELETE_BULK_CONFIRM_REQUIRED]`），而 `make` 把这些失败**标成
`Error 1 (ignored)`、退出码仍是 0**，日志里看起来就像「清干净了」。后果是紧接着的
`make` 报 `Nothing to be done`——**168 个 `.o` 一个没删，「全量重编 0 warning」这句话
就成了一句没有编译支撑的空话**。正确做法：不要靠 `make clean` 触发重编，改用
**刷新源码 mtime**（`find Code -name '*.cpp' -not -path 'Code/Tests/*' -exec touch {} +`）
再 `make`——既绕开删除守卫，又不删任何文件。另外 `make -B` 也不行：它会顺手强制
app bundle 的 `PkgInfo` / `Info.plist` 规则，而那两条规则开头就是 `$(DEL_FILE)`，
同样被守卫拦住 ⇒ 构建在**打包**这一步报 `Error 1`，而**编译当时才刚起了 6 个翻译单元**
（`make: *** Waiting for unfinished jobs....`），所以那一次的 warning 数只覆盖这 6 个文件，
**不能拿它当「全量重编 0 warning」的证据**。

### 1.42 VCS-001 后端抽象层：抽象类早就在，缺的是「谁在守它」的那三条（2026-09-24 04:5x）

**这一条此前为什么一直停在「待实现」**：`vcsbackend.h` 里的抽象类、`GitBackend`、
可注入假后端的 `VcsView`，以及一条真的在无 git 环境下跑的用例
（`missingGitDegradesAllQueries`）**都早就在**。核对完成标准时发现五条里有三条是空的，
但它们一条也不在「模块不存在」那一类里，而在**「模块在、没人守」**那一类里：

| 完成标准 | 核对前的真实状态 |
| --- | --- |
| ① 定义后端接口七件套 | **已在**（`detectRepo` / `status` / `log` / `diff` / `blame` / `revisionGraph` / `catFile` 全在抽象类上） |
| ② 首个实现调用外部 git 进程 | **已在**（`GitBackend` 全程 `QProcess`，独立参数、`GIT_*` 清洗、超时与输出上限；**没有任何仓库访问库**，所以也不产生新的许可结论） |
| ③ 无 git 时优雅降级（命令置灰 + 「未检测到 git」） | **空**。`availability()` 在仓库里**从来没有被生产代码调用过**——它是「定义了但没人问」的接口，`VcsView` 报错是运行时弹一句，命令本身照样可点 |
| ④ 探测结果按路径缓存，路径变化时失效 | **完全没有**。仓库里搜不到任何缓存：切一次模式、切一次路径就重探一次 |
| ⑤ 后端可被假实现替换 | **已在**（`setBackend()`），但「可以在没有 git 的环境里测」这条此前只有一条用例在用，缓存那条新增之后才真正成为**结构性**的能力 |

**本轮补的三件事，以及每件「谁在守」**：

1. **新模块 `Services/Vcs/vcsavailability.{h,cpp}`**——把「这条命令现在能不能用」与
   「同一个路径不要反复去问后端」这两个**同一问题的两侧**收在一个文件里（第 ③ 与第 ④ 条标准）。
   `isVcsActionId()` 是「哪些命令属于版本控制」的**唯一定义**：主程序与用例都从这里取清单，
   而不是各自抄一遍 `vcs.` 前缀——抄一遍的话，将来加第四条 VCS 命令时清单会分叉，
   而分叉的表现是「新命令不会变灰」，没有任何东西会红。
   `probeAvailability()` 把后端的 `availability()` 折成 `{是否可用, 不可用原因}`，
   并把「没装 git」翻译成用户看得懂的那一句；翻译只做一次（`unavailableReason()`），
   判据不在调用方。
2. **`RepositoryCache`**——按**规范化绝对路径**分桶、只缓存**定论**（成功或「不是仓库」）、
   探测在**锁外**做（锁里只取后端指针与自增的 `generation`），拿到结果后先比代次再写：
   探到一半后端被替换时，那个结果属于**上一个**后端。这三条各有自己的用例
   （`repositoryCacheNeverAnswersForADifferentPath` / `repositoryCacheCachesOnlyConclusiveAnswers`
   / `repositoryCacheDropsResultsFromAReplacedBackendMidProbe`）——最后那条在单线程测试里
   永远复现不出来，是靠假后端的 `duringDetect` 钩子**在探测中途换掉后端**才造出来的输入。
3. **主程序的接线**——`MainWindow::updateCommandState()` 的末尾按 `isVcsActionId()` 把三条
   VCS 命令一起 `setEnabled(可用, 原因)`；`setVcsBackend()` 让用例把假后端塞进**真窗口**
   （用例用的是真的 `MainWindow.cpp`，不是替身）。

**自审扫掉的一处死代码（值得记）**：最初还在 `MainWindow::showVcs(int mode)` 开头加了一道
「不可用就直接返回」的守卫，看起来是纵深防御。但 `VcsView` 的**唯一**入口是
`CommandRegistry::trigger`，而命令被置灰时那条路根本走不到 ⇒ 这道守卫**不可能被执行**。
删掉它并在 `updateCommandState()` 里留注释说明「为什么只有一处」。这与 §1.38 删掉
`applyTierToControls(tier, fromUser)` 是同一件事：**看起来像防线的第二处写入，实测是遮蔽防线**。

**验证（全部本机实跑）**：

| 验证项 | 实测结果 |
| --- | --- |
| 三个相关套件 | `Vcs` 68 / `VcsBlameView` 23 / `VcsView` 17 ⇒ 合计 **108 passed / 0 failed** |
| 全量测试 | **3775 passed / 0 failed / 2 skipped，71 个套件，EXIT=0**（3775 = 3763 + 12 = `Vcs` 58 → 68 的 10 + `VcsView` 16 → 17 的 1 + `AppIntegration` 13 → 14 的 1，**没有删改任何既有用例**） |
| 五道护栏 | 全部 `exit=0`：`check_layering` 分层通过 / `check_winapi` **353** 个源文件（新增两个）/ `check_spec` 369 条规格、P0 59 条 / `check_icons` 41 个图标 / `check_shell` 16 个脚本。**`exit=` 取自护栏程序本身**，不是管道末端 `tail` 的 |
| 主程序构建 | **全量重编 106 个翻译单元、0 条本仓 warning、0 error**，1 次链接。触发重编用的是**刷源码 mtime**（`find Code -name '*.cpp' -not -path 'Code/Tests/*' -exec touch {} +`），不是 `make clean`——理由见 §1.41 那条工具坑 |
...
| 变异（反向验证） | **19 处变异 19 处检出、0 漏检、0 无效**。该驱动跑了两遍：**第一遍 15 处检出 / 4 处待查**，逐条定性后**补掉两处真缺口**（M14「换后端不清空缓存」、M03「置灰原因只剩裸 detail」）、另两处列进 `TOLERATE` 并写明理由（M09 / M13 与预期用例守的是同一件事）。19 处打在 `vcsavailability.cpp` / `vcsview.cpp` / `MainWindow.cpp` 上，分布到 `Vcs` / `VcsView` / `AppIntegration` 三个套件；驱动 `/tmp/lq-vcs-mutate.py`（**不进仓库**）。每处都**在还原源码之前**比对目标 `.o` 的 mtime 来证明变异真的编进去了，构建失败（没有统计行）一律按「无效」而不计漏检 |

**本轮踩到的两条新坑（已进 §6）**：
- **`isBusy()` 不能当「第一次加载已经做完」用**。新加的 `VcsView` 缓存用例原来写
  `QTRY_VERIFY(!view.isBusy())`，而 `isBusy()` 在**任务派发之前**就是 false ⇒ 断言立刻通过、
  假后端的 `detectCalls()` 还是 0，失败信息看起来像「缓存没生效」。改成等**可观测的后端计数器**
  （`QTRY_COMPARE(backend->detectCalls(), 1)`）才是真正的「等它做完」。
- **`RepositoryCache c(QSharedPointer<Backend>());` 会被解析成函数声明**（most vexing parse），
  报错是 `member reference base type 'RepositoryCache (QSharedPointer<Backend> (*)())' is not a structure or union`，
  与「类型用错了」的提示差得很远。写成具名局部量再传进去即可。
- 另有一条不是坑但会被误判成坑的：**四个 worker lambda 共用一个签名**，其中三个用不到缓存参数，
...
| `e9ff548` | **崩溃套件的调用栈捕获**（§1.40，新功能，**没有一行生产代码**；这一轮跑到一半被强杀、**由下一轮接手验完才提交**，接手经过见 §1.41）：`Code/Tests/run-tests.sh` 在「崩溃」这条判据上（与 `-v2` 补跑同一个判据：非超时 + 没产出 `Totals:` 行）再走一步——用平台上的调试器把同一个二进制重跑一次并打印调用栈，产物 `crash-trace.txt`（首轮退出码 / 调试器 / 完整命令行 / 看门狗秒数 / 原始输出 / **我们自己写的结论行**）。三个新函数：`select_crash_debugger()`（显式 `LQCOMPARE_TEST_DEBUGGER` 优先，否则 `command -v` 探 gdb → lldb）、`crash_trace_argv()`（gdb 是 `-batch -ex run -ex bt 40 --args`、lldb 是 `--batch -o run -o bt -c 40 --`，按可执行文件名选形式表，因为**写岔了的症状是静默拿到空表**）、`capture_crash_trace()`。三条纪律：与补跑同判据、**不传 `-o`**（传了等于让诊断改写证据，改掉的正是首轮唯一的现场）、超时下限 `max(TIMEOUT, 60)`（调试器要先启动；本机实测真 lldb 就是靠这条下限被终止并照实写下「没拿到调用栈」）。**没有可用调试器时照样写下产物**并明说拿不到栈，「产物存在」因此始终等价于「这个套件崩过」。自测 32 → **46 条断言**（随后自查补上「日志帧清单」那 2 条，终值 **48 条**）、探针 5 → 6 个（新增 `ZZProbeAbort`：复位 `SIGABRT` 处置再 `raise`，用 `raise` 而非 `abort()` 是因为后者是 `noreturn`、`-O2` 下可能被编译成尾调用而抹掉那一帧），另加两遍只跑崩溃探针的调用（过滤 + 独立构建根目录）验 lldb 旗标形状与「本平台没有调试器」；CI 上传 `crash-trace.txt` 并预先打印调试器可用性（没有则 `::warning::`）。**15 处变异 14 处检出、1 处按设计不漏检、0 处漏检**（接手后本机重跑，见 §1.41）；真 gdb 那一支**未在本机实测**（本机没有 gdb，且 lldb 调试不通，见 §6） | ENG-003 / ENG-004 |
...
### 4.0.18 本轮（2026-09-24 01:2x）：崩溃套件的**调用栈**捕获（ENG-003 / ENG-004 的基础设施，**新功能**）

- **第 0 步判锁**：**没有锁文件**（上一轮按规矩删掉了），`pgrep` 里没有别的
  `--session-id`，工作树干净、`HEAD` 与远端 `main` 逐字一致、`.workbuddy/wip/` 为空。
  12 分钟窗口内确实有 6 个文件被改过——但**那正是上一轮 00:52 / 00:53 那两个提交
  收走的产物**（`statuspalette.*` / `tst_statuspalette.cpp` / `foldercompareview.cpp` /
  两份文档），没有任何半成品。按本项目已确立的判据（§1.28 那一轮：**先看持有进程
  是否还活着，别把「压缩后续跑的同一轮」误判成「有人在做」而白白跳过**）开工，
  并且**立刻取锁**把并发交给锁仲裁（`pid=$PPID`；沙箱里 `ps` 被禁，`$$` 拿到的 pid
  立刻变死、会被误判成死锁）。**收工前已删锁。**
- **这一轮只做一件事**：把 §1.36 的候选动作 2（**要一份调用栈**）落地。
  ubuntu 腿的 `Tests/Folder` 崩在 `Folder::compare()` 那一串调用里，在拿到栈之前
  **不许改 `foldercompare.cpp`**——凭猜改一处的代价是「改错 + 一次 CI 往返」。
- **交付**（只动两个文件，**没有一行生产代码**）：
  - `Code/Tests/run-tests.sh`：`select_crash_debugger()`（显式 `LQCOMPARE_TEST_DEBUGGER`
    优先，否则 `command -v` 探 gdb → lldb）、`crash_trace_argv()`（**两个调试器的旗标
    完全不同**，按可执行文件名选形式表）、`capture_crash_trace()`（产物
    `crash-trace.txt`：首轮退出码 / 调试器 / 完整命令行 / 看门狗秒数 / 原始输出 /
    **我们自己写的结论行**）。`run_binary_with_timeout()` 增加第一个参数「本次超时秒数」
    （三处调用：主运行、`-v2` 补跑、抓栈），抓栈用 `max(TIMEOUT, 60)`。
  - `.github/workflows/build.yml`：测试步骤先打一行「崩溃栈捕获可用调试器：…」（没有就打
    `::warning::`，**不让腿变红**）；测试日志产物加上 `_test-build/**/crash-trace.txt`。
- **三条纪律**（每条都对应一个会真实发生的错法）：① 与 `-v2` 补跑**同一个判据**
  （非超时 + 没产出 `Totals:` 行）；② **不传 `-o`**（传了就是让诊断改写证据，
  改掉的正是首轮唯一的现场）；③ **超时下限 60 秒**（调试器要先启动，自测窗口只有 5 秒）。
  **没有可用调试器时照样写下产物**并明说拿不到栈——「产物存在」因此始终等价于
  「这个套件崩过」，「没有栈」永远是一句写出来的结论。
- **测试**：`run-tests.sh --self-test` **32 → 46 条断言**（自查补上「日志帧清单」那 2 条后为 **48 条**）；探针 5 → **6 个**
  （新增 `ZZProbeAbort`：复位 `SIGABRT` 的处置再 `raise`，因此每个平台都走崩溃路径）；
  另加**两遍只跑崩溃探针**的调用，各配独立构建根目录（共用会覆盖上两遍的产物证据）。
- **验证**：**15 处变异 14 处检出、1 处按设计不漏检、0 处漏检**（接手后在 §1.41 那一轮本机重跑确认）；
  五道护栏全绿；本机还端到端跑了一次**真 lldb** 作对照（详见下面「本轮最值钱的一条」）。
- **本轮最值钱的一条（同族通用）**：**要验「工具被怎么调用」这件事，断言要由被调用方
  记录，不能由调用方自己打印再自己断言**。本机 lldb 调试不通（见 §6），于是自测里
  gdb 的形状只能靠一个「假调试器」来收——它把收到的 argv 原样写进产物，
  断言才有意义（`STUB-DEBUGGER-ARGV: -batch -ex run -ex bt 40 --args …`）。
  这一支的代价是**真 gdb 只能等 ubuntu 腿第一次跑到崩溃才知道**，所以文档里必须
  如实写成「未在目标平台验证」，而不是含糊成「已验证」。
- **下一轮从哪里接**：**下一次 CI 的 `crash-trace.txt` 就是答案**——它会给出
  `Folder::compare()` 那一串调用里的具体某一帧（帧名来自符号表，注意测试工程是
  `-O2` 且**没有 `-g`**，`static` 函数可能只有地址）。拿到之后按 §1.36 的候选动作 1
  （二分夹具：把 `cycle` 那两个目录符号链接去掉只留 `dangling`）或直接改代码走。
  在那之前，`DIR-002` ~ `DIR-012` 剩的那七条（`002` / `004` / `005` / `006` / `007` /
  `009` / `010`）里 `DIR-005` 只缺界面入口、`DIR-004`（文件大小）与 `DIR-002`
  （大小写与规范化）都还没独立核对过；`TXT-011` / `015` ~ `017` / `004`、
  Windows 腿 26 个套件构建失败、`actions/*@v4 → v5` 仍在队列里。

### 4.0.19 本轮（2026-09-24 02:5x）：**接手**上一轮被中断的会话，把 §1.40 验完并提交（ENG-003 / ENG-004）

- **第 0 步判锁**：`run.lock` **存在**且 `pid=43189` **已死**（`kill -0` 失败；
  `heartbeat` 从写入起**一次都没刷新过**，停在 `started=2026-09-23T16:57:12Z`），
  `pgrep` 里没有别的 `--session-id`，`Code/` `docs/` `tools/` 下 12 分钟内无改动，
  `git log -1` 在两小时前（`000b164`）⇒ 判为「**上一轮被中断**」并**接手**（pid 写 `$PPID=57069`）。
  工作树里有 4 个文件、551 行插入未被提交——**「工作树是脏的」不构成跳过的理由**，
  它正是上一轮没做完的活。**收工前已删锁。**
- **接手动作**：逐文件核对「这是可用的半成品还是错误状态」⇒ 判定为**可用的成品**
  （三个新函数齐全并已接线、16 条自测断言齐全、文档连 §6 坑表都写了），
  因此**不回滚任何文件**，直接在它之上做完整验证。
- **先证明「没有变异残留」**：`Code/Tests/run-tests.sh` 与上一轮驱动留下的
  `/tmp/lq-run-tests-pristine.sh` **逐字节相同**（sha256 均为 `c742e43…`）。
  这一步是本轮的第一道验证，因为**「没有残留」不能用枚举变异特征来证明**（§6 那条）。
- **交付与验证**（全部本机实跑，明细见 §1.41 的表格）：
  `--self-test` **48 条断言全绿**；全量 **3763 passed / 0 failed / 2 skipped（71 个套件）**；
  五道护栏全部 `exit=0`；主程序**全量重编 105 个翻译单元 0 warning** + 重签 + 离屏三行日志；
  变异 **15 处 / 14 处检出 / 1 处按设计不漏检 / 0 漏检**（本机重跑）。
- **顺手改掉两处文档偏差**：① `architecture.md` 的 `Tests/` 行「3762 条」→ **3763**
  （实现测出来的值，只改数字不改形状，免得撞 `check_spec.py` 的保留写法）；
  ② 把 §1.40 那句「全量与主程序构建的数据见下面那张表」改成指向 §1.41 的真表
  ——**原来那句指向一张并不存在的表**，是本轮接手时发现的第一个「说了但没做」。
- **发现的工具坑（已进 §6）**：全量测试之后 `make clean` **静默失效**
  （批量删除守卫按轮累计，而 `make` 把被拦的 `rm -f` 标成 `Error 1 (ignored)`、退出码仍 0），
  于是 `make` 报 `Nothing to be done`、**「全量重编」一次都没编**。正确做法是
  **刷新源码 mtime** 而不是删 `.o`，并且把判据换成「`make` 退出码 0 **且**有上百个翻译单元被编译」。
- **下一轮从哪里接**：**下一次 CI 的 `crash-trace.txt` 就是 ubuntu 崩溃线的答案**
  （它会给出 `Folder::compare()` 那一串调用里的具体某一帧）。拿到之前**不要改
  `foldercompare.cpp`**（§1.36）。在那之前，队列是：`DIR-005`
  （只缺开关与界面入口，服务层已就位，本机可完整闭环）、`DIR-004` / `DIR-002`
  （都还没独立核对过）、`DIR-006` / `007` / `009` / `010`、`TXT-011` / `015` ~ `017` / `004`、
  Windows 腿 26 个套件构建失败、`actions/*@v4 → v5`。
- **停下的原因**：不是「没有可做的条目」，而是本轮**接手 + 从零重验**（自测 + 全量 +
  五道护栏 + 主程序全量重编 + 15 处变异）已经用掉一整轮预算。停在 §1.40 那条
  **完整闭环之后**，未留半成品。

### 4.0.20 本轮（2026-09-24 04:5x）：VCS-001 后端抽象层的「谁在守它」（issue #272，**已完成**）

- **第 0 步判锁**：无锁文件，`Code/` `docs/` `tools/` 12 分钟内无改动，
  `git log -1` 在一小时前（`09cb7f1`），`pgrep -fl "run-tests|qmake|make -j|WorkBuddy"`
  里没有别的 `--session-id` ⇒ 判为可开工，按 `pid=$PPID` 取锁（沙箱里 `ps` 被禁、
  `$$` 拿到的 pid 立刻变死，会被下一轮误判成死锁）。**收工前已删锁。**
- **本轮做的是队列里最靠后那一组，而不是第 ① 项**：prompt 里的 ① 是 ubuntu CI 的
  `Tests/Folder` 缓冲越界崩溃线，而它的下一步依赖**新一次 CI 的 `crash-trace.txt`**；
  §1.40 / §1.41 两轮已经把「哪一条用例」推进到了「哪一行」，**再往前必须有新 CI 证据**
  （本机没有 gdb，lldb 调试不通，ASan 那一路在本轮预算内排不进来）。因此本轮从
  队列里挑了一条**完全不需要平台证据**的 P0：VCS-001。
- **为什么是 VCS-001 而不是别的**：按「完成标准里有没有动词指向一个还不存在的模块」
  逐条筛过剩下的 P0，VCS-001 五条标准一条都不指向外部模块；`ls` 源码与测试目录之后
  发现它是**「模块在、三条标准没人守」**那一类（详见 §1.42 的核对表）——这类最值得做，
  因为它的价值全在**能不能被反向验证**上，而这一点本机就能闭环。
- **交付**：新模块 `Services/Vcs/vcsavailability.{h,cpp}`（命令可用性的唯一定义与唯一
  翻译处 + 按规范化绝对路径分桶、只缓存定论、锁外探测 + 代次校验的 `RepositoryCache`）；
  `MainWindow` 按 `isVcsActionId()` 统一置灰三条 VCS 命令并提供 `setVcsBackend()`；
  `VcsView` 跨模式切换保留缓存、显式刷新失效。**顺带删掉 `showVcs()` 开头那道
  不可达的守卫**（唯一入口是命令注册中心，置灰时走不到），理由见 §1.42 与 §6。
- **测试**：`Tests/Vcs` 58 → 68、`Tests/VcsView` 16 → 17、`Tests/AppIntegration` 13 → 14
  （**没有新增套件**）；全量 3763 → **3775 passed / 0 failed / 2 skipped（71 个套件）**。
- **验证**：五道护栏全部 `exit=0`（`check_winapi` 351 → **353** 个源文件）；
  主程序**全量重编 106 个翻译单元 0 warning**（用刷源码 mtime 触发，不用 `make clean`）
  + 重签 + 离屏三行日志；变异测试的结果见下一条。
- **变异测试（反向验证）**：**19 处变异 19 处检出、0 漏检、0 无效**（驱动 `/tmp/lq-vcs-mutate.py`，
  **不进仓库**；19 处打在 `vcsavailability.cpp` / `vcsview.cpp` / `MainWindow.cpp` 上，
  分布到 `Vcs` / `VcsView` / `AppIntegration` 三个套件）。**第一遍是 15 处检出 / 4 处待查**，
  逐条定性之后分成两类，两类都值得记：
  - **两处是「守的其实是同一件事」**（M09 / M13），列进驱动的 `TOLERATE` 并写明理由：
    前者（「只缓存定论」）在专用夹具之外还有一个可观察面——多路径那条夹具里「没有后端」的
    实例返回的**正是** `Unavailable`，它一旦入缓，`cachedPathCount()` 就从 0 变 1；
    后者（「`invalidate()` 必须真的丢掉那一条」）被 `VcsView::refresh()` 用着，于是服务层与
    视图层同时变红。
  - **另两处是真缺口，补断言之后才变红**：
    **M14**（`setBackend()` 里那句 `m_entries.clear()`）——原用例在它**紧前面**刚调用过
    `cache.clear()`，于是「换后端之后计数为 0」是一句**恒真**的话（那个 0 是上一步的功劳），
    必须先**把缓存重新填上**再换后端才可观察；这正是本仓那条「断言钉住了旁边的量」的又一种写法。
    **M03**（`unavailableReason()` 末尾那句带主语的聚合文案）——原断言只钉了「原始 detail
    在里面」，把整句换成裸 detail 一样绿，于是「置灰原因必须**自带主语**」这条设计意图
    （写在实现注释里）**无人守**；补了「不得等于裸 detail」与「必须含『版本控制』」两条。
- **下一轮从哪里接**（顺序不变，只把 ① 的前置条件写清）：
  ① ubuntu `Tests/Folder` 崩溃线——**下一步只能是读新一次 CI 的 `crash-trace.txt`**，
  在那之前不要动 `foldercompare.cpp`；② Windows 腿 26 个套件构建失败（`_WIN32_WINNT` 与
  `winioctl.h` 两条系统性原因先做）；③ `actions/checkout@v4` / `upload-artifact@v4` → v5；
  ④ `TXT-011` / `TXT-015` ~ `TXT-017` / `TXT-004`；⑤ 剩下的 `DIR-002` ~ `DIR-012`；
  ⑥ UI 接线批次与 `OPT-002` 最小集。
- **停下的原因**：**不是「没有可做的条目」，而是这一轮已经把一条 P0 完整闭环**
  （实现 + 三个套件的用例 + 全量 + 五道护栏 + 主程序全量重编与离屏启动 + 变异测试 +
...
| **本机的 `lldb` 调试链路是**不通**的（不是「不会用」，是环境）** | 两条实测：① 对 **x86_64（Rosetta）**进程 —— 进程被信号带走时 lldb **不会停下来**，于是 `-o bt` 只回一句 `error: Command requires a process which is currently stopped`，在 `abort` 上打断点也**不命中**（`Breakpoint 1: 10 locations.` 之后进程照样以 `status = 6` 收场）；② 对**原生 arm64** 进程 —— lldb 直接**卡在 `run` 上不返回**（`debugserver --native-regs --setsid --fd=9` 起来之后没有下文，同一台机器上 `clang -O0 abort.c` 那种最小程序也一样）。本项目的 Qt 5.15.2 是 x86_64，所以「在 mac 上抓一次崩溃栈」这条路本机走不通 | 本轮的自测因此**不依赖真调试器**：用「假调试器」（`STUB-DEBUGGER-ARGV: …`）验运行器这一侧的契约，真调试器那一支只能等 ubuntu 腿（gdb）。**在 mac 上跑真 lldb 的后果是 `crash-trace.txt` 里写一句「调试器在 60s 内没有结束，已被看门狗终止」**——那是如实的结论，不是 bug。别再花时间试 `-k bt` / `process handle SIGABRT --stop true`：两者都只在「进程被停下」时才起作用，而问题就出在「停不下来」 |
| **把「外部工具被怎么调用」写成断言时，记录方必须是被调用方** | 运行器要调 `gdb` / `lldb`，而这两者在本机都不可用（见上一行）。如果让运行器**自己打印一行**「我调的是 `gdb -ex bt`」再断言那一行，那么「形状写错了」这件事**永远不会被发现**——断言只是把实现重念了一遍 | 自测里写一个几十行的**假调试器**，它把自己收到的 argv 原样打进产物（`STUB-DEBUGGER-ARGV: -batch -ex run -ex bt 40 --args …`），断言去读**被调用方记下的**那一行。代价必须写进文档：真 `gdb` 那一支**未在目标平台验证**，只能等 CI 第一次崩。它的行为也要照真调试器分岔（被信号带走就出帧，否则直说没现场），否则「跑完了但没拿到栈」那一支永远走不到 |
| **`lldb` 与 `gdb` 的旗标不通用，而写岔了的症状是「静默拿到空表」** | 崩溃现场被调试器抓到之后，`gdb` 用 `-batch -ex run -ex bt 40 --args <bin>`；`lldb` 用 `--batch -o run -o bt -c 40 -- <bin>`。把 gdb 那套旗标交给 lldb，或者反过来，**都不会报错**——lldb 只回一句 error、gdb 会把它当成未知选项停在启动上，两种都不写调用栈，而在产物里看起来与「这次崩得没有现场」一模一样 | 写成一张**按可执行文件名选的形式表**（`crash_trace_argv()`），并且**两支各有一条断言**钉住；自测里那两遍「只跑崩溃探针」的过滤器调用就是为此加的。别用 `--version` 的输出去分辨两者：那要多起一个进程，而那个进程自己也可能挂住 |
| **诊断产物也必须进「每轮开头先删干净」那张清单** | `run_suite()` 在**任何动作之前** `rm -f` 上一轮的产物。新加的 `crash-trace.txt` 漏出这张清单时，症状是「这一轮没崩的套件带着上一轮的崩溃现场被上传」——下一个人会照着它去查一个**不存在**的崩溃。`verbose.txt` 早就在清单里，新产物极容易忘 | 加进 `rm -f`，并让自测**提前埋一份假的 `crash-trace.txt`**（照 `results.txt` 那条既有做法），断言它在本轮结束时不见了。只断言「崩溃的套件有产物」是看不出这个漏的 |
| **变异驱动被强杀（SIGKILL）时，变异会留在仓库里；而「逐条 grep 变异特征」这种做法一定会漏** | 本轮驱动在跑 M01（删掉抓栈调用）时被强杀，`finally` 没执行 ⇒ 仓库里留着那一处变异。而我事后的「残留检查」只 grep 了 M02 / M06 的特征字符串，**检查本身给出了绿灯**，于是把「crash-trace 断言全红」误读成了环境问题，白跑了一轮自测才定位到 | 驱动在**基线绿之后**先把自己那份内容存成 `/tmp/lq-run-tests-pristine.sh` 并打印 sha1，核对/还原一律以它为准；另外长任务**分批跑**（每批 5 处）并把结论逐处追加落盘，被强杀也不丢已完成的部分。教训：**「没有残留」这件事不能靠特征枚举来证明** |
| **本机的长命令会被环境的看门狗用 `lldb -p <pid>` 抓栈，而本机 lldb 自己会挂住 ⇒ 目标进程被冻住** | 观测：一次变异驱动卡了约 9 分钟没有进展，`pgrep -fl 'lldb -p '` 一次数出 **42 个** `lldb -p <pid>`（每个都配一个 `sh -c lldb -p <pid> <<EOF bt all quit EOF`）；被附着的目标进程处于 SIGSTOP 状态，连带把长任务的后代一起拖住。这不是本仓代码的问题，但它会把「跑不动」伪装成「实现坏了」 | 发现卡住时先清掉这些挂住的 `lldb`（`kill -KILL`）再继续；长任务**分批 + 结论落盘**，不要指望一条命令跑十几分钟；判定「是不是环境问题」时先看 `pgrep` 里有没有成堆的 `lldb -p` |
| **「同一件事产物里有了、日志里也有一份」时，只守产物就等于把那半份日志留成死代码** | 本轮实测：`capture_crash_trace()` 末尾那段「调用栈 N 帧 + `\| ` 前缀贴出最外层几帧」的**运行器日志**没有任何断言守着——把整段删掉，补上前的 46 条断言**全绿**。而这段日志正是「不必下产物就能看到」这句承诺的全部兑现，删除它不会有人发现 | 按「每处实现都要问『谁能把它打红』」逐分支扫一遍（`break` / `continue` / 早退 / 工厂缺省 / **日志分支**都算），发现没人能打红的分支就补断言、并加一处变异反向验证（本轮 M15）。**「产物里有了」不是「日志那半份可以不管」的理由**——两者是给不同的人看的（一个是下产物的人，一个是只看 CI 日志的人） |
| **变异驱动报「检出（但预期的那几条没红）」时，先分清是「覆盖不足」还是「预期标注写偏」** | 本轮 M06（帧正则改成永不匹配）与 M09（删掉降级块）都落进这个分类，但两处各有断言真变红：M06 红在**结论行**（帧数为 0 ⇒ 结论从「拿到了」翻成「没拿到」），M09 红在**降级文案**与**环境变量提示**。真正错的是我在驱动里写的 `expect` 列表——期待的是「断言名看着该红的」，而不是「它实际依赖哪个可观测后果」。**两者的修法完全相反**：覆盖不足要补断言，标注写偏只改驱动 | 报告里的 `缺=[…]` 与 `越界=[…]` 要逐条读一遍再下结论，别只看「检出」两个字；发现标注写偏就顺手把 `expect` 改准（本轮改了 M06 / M09），否则下一轮会照着旧标注继续误判 |
| **全量测试之后 `make clean` 会静默失效，于是「全量重编」一次都没编** | 沙箱的批量删除守卫**按轮累计**（阈值 50）：全量测试的产物清理早就把配额用掉，随后 `make clean` 的每一条 `rm -f` 都被拦，而 `make` 把失败**标成 `Error 1 (ignored)`、退出码仍是 0**，日志里看着就像「清干净了」。紧接着 `make` 报 `Nothing to be done`，**168 个 `.o` 一个没删**，「全量重编 0 warning」成了一句没有编译支撑的空话；`make -B` 也不行——它强制 app bundle 的 `Info.plist` / `PkgInfo` 规则，而那两条开头就是 `$(DEL_FILE)`，同样被拦，构建在**打包**这一步红掉（编译当时才起了 6 个文件） | 不要靠 `make clean` 触发重编。改用**刷新源码 mtime**：`find Code -name '*.cpp' -not -path 'Code/Tests/*' -exec touch {} +` 再 `make`——绕开删除守卫、不删任何文件。判据也要跟着换：**「构建通过」必须看到 `make` 退出码 0 且有上百个 `.cpp` 被编译**（本轮 105 个），只有 `Nothing to be done` 或 `Error 1 (ignored)` 都不算 |
| **`isBusy()` 之类「忙碌」标志不能当「第一次加载已经做完」用** | VCS-001 新增的视图缓存用例原来写 `QTRY_VERIFY(!view.isBusy())`，指望它代表「第一次探测已经回来」。但 `isBusy()` 在**任务派发之前**就是 `false`（它是一个状态标志，不是「至少跑过一次」的标志），于是断言**立刻通过**，假后端的 `detectCalls()` 还是 `0`，紧接着的 `QCOMPARE(..., 1)` 报 `Actual: 0` ——失败信息看起来像「缓存没生效」，指向的是完全错误的方向。同族的还有「等窗口出现」「等 model 非空」这类**在动作发生前就已成立**的前置条件 | 等待**只能等一个由被测行为产生的可观察量**：本轮改成 `QTRY_COMPARE(backend->detectCalls(), 1)`（以及 `diffCalls().size()` / `logCalls().size()`），假后端自己数次数，与派发时机无关。一般规则：`QTRY_*` 的条件必须**在动作发生前为假**，否则它是一条恒真的断言；写之前先问一句「如果我把它删掉、直接跑下一行，会不会红？」 |
| **`RepositoryCache c(QSharedPointer<Backend>());` 会被解析成函数声明（most vexing parse），报错完全指不到真正的原因** | 新增缓存用例要造一个「没有后端」的实例，写成 `RepositoryCache c(QSharedPointer<Backend>());`——编译器把它读成「声明一个函数 `c`，参数是一个指向「返回 `QSharedPointer<Backend>` 的无参函数」的指针」，于是**变量根本不存在**，报错落在下一行：`member reference base type 'RepositoryCache (QSharedPointer<Backend> (*)())' is not a structure or union`，看起来像类型用错了 | 先声明一个具名局部量再传进去：`const QSharedPointer<Vcs::Backend> noBackend; RepositoryCache c(noBackend);`。**凡是实参本身是「类型名 + 空括号」形状的构造，都要怀疑这一条**（`QVector<int>()` 这类内置类型不会，但任何用户类型的 `T()` 都会）。同一族的还有 `QSet<QString> s{};` 与 `Foo f(std::string())` |
| **四个 worker lambda 共用同一个签名时，用不到的形参在 `-Wextra` 下会报 `-Wunused-parameter`** | `VcsView` 的四个工作 lambda 共用 `std::function<WorkResult(const Backend &, RepositoryCache &, const std::atomic_bool *)>`，其中三个用不到缓存。留着参数名 ⇒ 全量重编冒出 3 条 warning（`vcsview.cpp:378/401/433`），而本项目的验收标准是**0 条本仓 warning** | **删掉形参名、保留类型**（`RepositoryCache &` 不写名字），并在注释里说明「签名要求这个参数存在，但这一支不需要它」——这样既保持 0 warning，也不会让下一个人以为参数被漏写了。不要用 `Q_UNUSED` 糊过去：那样 warning 没了，但「这个 lambda 用不到它」这件事实也一起被藏掉了 |
