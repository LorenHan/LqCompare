...
| 测试（全量） | **3601 passed / 0 failed / 2 skipped，66 个套件**（2026-09-21 11:52 实测，**并行 4 时 1 分 26 秒**，EXIT=0）。3601 = 3598（ENG-003 那一轮，见 §1.24）+ 3（TXT-002 本轮新增的三个用例，见 §1.25），**没有删改任何既有用例**。并行的收益是 233s → 81s（串行基线 3 分 53 秒）。2 条跳过分别来自 `PathName`（40/0/1）与 `Registry`（61/0/1），都是按平台条件跳过的用例 | `Code/Tests/run-tests.sh` |
