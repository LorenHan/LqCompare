...
| 测试（全量） | **3615 passed / 0 failed / 2 skipped，67 个套件**（2026-09-21 13:0x 实测，**并行 4 时 1 分 20 秒左右**，EXIT=0）。3615 = 3601（TXT-002 那一轮，见 §1.25）+ 14（TXT-003 本轮新增的 `Tests/Alignment` 套件，见 §1.26），**没有删改任何既有用例**。并行的收益是 233s → 81s（串行基线 3 分 53 秒）。2 条跳过分别来自 `PathName`（40/0/1）与 `Registry`（61/0/1），都是按平台条件跳过的用例 | `Code/Tests/run-tests.sh` |
...
> 本轮新增的 `5c16735` 与 `83a1c8f` 走的是同一条路（提交号写在本节的最后一行，
> 由紧随其后的纯文档提交定稿）。
