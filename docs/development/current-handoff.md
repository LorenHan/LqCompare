...
| 测试（全量） | **3653 passed / 0 failed / 2 skipped，68 个套件**（2026-09-21 17:4x 实测，并行 4 时约 1 分 30 秒，EXIT=0）。3653 = 3649（TXT-005 那一轮，见 §1.29）+ 4（TXT-001 本轮：`Tests/TextView` 20 → 24，四条完成标准各一条用例函数），**没有删改任何既有用例**。2 条跳过分别来自 `PathName`（40/0/1）与 `Registry`（61/0/1），都是按平台条件跳过的用例 | `Code/Tests/run-tests.sh` |
