...
1. **`Archive` 在 ubuntu 与 macOS 上都失败，同一个原因**：它的 `initTestCase()` 调
   `generate_fixtures.py` 生成夹具并回读校验，而其中 `cp437.zip` 是**故意**用 cp437 名字
   （`RawName(b"caf\x82.txt")`，不带 UTF-8 标志位）写出来的；CI 运行器的 **Python 3.14**
   读它时抛 `UnicodeDecodeError: 'utf-8' codec can't decode byte 0x82`（本机 3.13 与 3.9 都不抛）。
   夹具本身没错，是「用 stdlib 读者去校验一个故意非 UTF-8 的夹具」这一步在 3.14 上不再成立。
   **修复方向**：把 `cp437.zip` 从回读校验的名单里去掉（改为按原始字节校验），
   并在夹具生成器里写明它依赖的 Python 行为。
...
上面第 1、2 条是**下一轮**的活；第 3、4 条已在本轮随 `run-tests.sh` 一起修掉并做了反向验证
（见 §6）。这些失败**都不是本次改动引入的**——它们一直存在，只是以前没有任何机制把它们摆出来。
...
> `a1cd3ee` 与 `786cf46` 这十六行由**单独的纯文档提交**补写提交号，
...
1. **修 `Code/Tests/Archive` 的夹具校验**（优先级最高）。它在 ubuntu 与 macOS 两条腿上都红，
   原因是 `generate_fixtures.py` 用标准库读者回读一个**故意非 UTF-8** 的夹具
   （`cp437.zip`），CI 的 Python 3.14 会抛 `UnicodeDecodeError`，本机 3.13/3.9 不抛。
   改法：把该夹具从回读校验名单里去掉，改为**按原始字节**校验，并在生成器里写明它依赖的
   Python 行为。**这是三条腿里唯一一处「只要修掉就能让某个平台变完整」的红**，性价比最高。
...
| `ENG-004` 持续集成流水线 | ~~「没有 CI」~~、~~「从来没跑过构建与测试」~~ 均已修；**三条腿的真实结果也已拿到（run `35546164218`）** | 四个阶段、三平台矩阵、产物上传都已写进 `.github/workflows/build.yml`；`run-tests.sh` 的**七项**跨平台问题已修（见 §1.20）。第一次真跑的证据：三条腿都跑完了四个阶段、上传了 6 个 artifact，**流水线本身按设计工作**（拿不到 LqRibbon 时正确跳过主程序构建与打包、测试照跑）。**但三条腿现在都是红的，红的不是流水线而是被测对象**：`Archive` 套件在 ubuntu/macOS 上因 CI 的 Python 3.14 抛 `UnicodeDecodeError`（第 1 条，真 bug，下一轮修）；ubuntu 17 个 / Windows 26 个套件构建失败（第 2 条，`build.log` 已上传，下一轮读它定位）；Windows 的 8 个套件没产出 `Totals:` 行。另有一处**结构性限制**：主程序构建、打包、主程序产物上传这三步都要私有仓 LqRibbon，没有 `MYCLASS_TOKEN` 时在公开 CI 上永远走不到，Windows 腿的打包分支（`7z`）因此是**未验证代码**。`ENG-003` 还差的两项（单套件超时、并行执行）不属本条 |
...
| **CI 上的 Python 版本与本机不同** | `Tests/Archive` 的 `generate_fixtures.py` 生成夹具时会**回读校验**，其中一个 `cp437.zip` 是**故意**用 cp437 名字写的（`RawName(b"caf\x82.txt")`，不带 UTF-8 标志位）。本机 Python 3.13 / 3.9 读它都不抛，CI 运行器的 **3.14 抛 `UnicodeDecodeError`** ——于是这个套件在 ubuntu 与 macOS 两条腿上都红，而本机全绿。夹具本身没错，错的是「用一个会做编码推断的标准库读者去校验一个故意非 UTF-8 的夹具」 | ① 校验这类夹具要**按原始字节**比，不要经过会解码的读者；② 生成器里写明它依赖哪个 Python 行为；③ 更一般地——**「本机全绿」推不出「CI 全绿」**，凡是走系统解释器/工具链的地方（Python、make、7z、shell）都要假设版本不同。**未修**，是下一轮的第一件事 |
