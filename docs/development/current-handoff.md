...
| `（待回填）` | 设置项的声明 / 草稿 / 询问策略 / 声明目录，以及按声明生成的设置对话框 | SESS-006 |

> 上面这张表里，PLAT-005、FILT-001、SESS-001 与 SESS-002 那四行由**单独的纯文档提交**
> 补写，SESS-006 那行同理——把一个提交的提交号写进它自己，会因为 `--amend` 每次都改变
> 提交号而永远对不上。
